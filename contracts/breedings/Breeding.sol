...
contract Breading is Ownable, IBreeding {
    ...
    uint256 private _randomPercent = 10;
    uint256 private _minArcane = 5;
    uint256 private _maxArcane = 10;
        ...
        returns (uint256)
    {
        ...
        uint32 generation = (GenesUtil.getGeneration(genes1) + GenesUtil.getGeneration(genes2)) / 2 + 1;
        ...
        // body parts
        for (uint8 i = 0; i < bodyPartsCount; i++) {
            ...
            if (_randomPercent < random(_globalSeed, randomIndex++, 0, 100)) {
                id = uint8(
                    random(_globalSeed, randomIndex++, minId, maxId + 1) & 0xFF
                );
            } else {
                if (random(_globalSeed, randomIndex++, 0, 100) >= 50) {
                    id = GenesUtil.getId(genes2, i);
                } else {
                    id = GenesUtil.getId(genes1, i);
                }
            }

            genes = GenesUtil.setId(genes, i, id);
            genes = GenesUtil.setLevel(genes, i, 1);
            genes = GenesUtil.setArcane(genes, i, isArc ? 1 : 0);
        }
    ...
    function getArcanePercent(
        ...
        return (1 * (generation * mult - min * mult)) / (max);
    }
}
